class MakeBBFuse extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Soak Rag in Gasoline";
		...
		//result1
		AddResult("BB_Fuse");//add results here
	...
	override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		if (Class.CastTo(ingredient1, ingredients[0]) && ingredient1.GetLiquidType() == LIQUID_GASOLINE )
		{
			return true;
		}
		return false;
	...
	{
		ItemBase theResult = ItemBase.Cast(results.Get(0));
		if (theResult){
			theResult.SetWetMax();
		}
	}
